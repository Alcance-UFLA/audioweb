...
CREATE TABLE IF NOT EXISTS `audioweb`.`imagens_regioes` (
  ...
  `tipo_regiao` ENUM('circ','rect','poly') NOT NULL,
...
CREATE TABLE IF NOT EXISTS `audioweb`.`secoes_textos` (
  `id_secao_texto` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `identificacao` VARCHAR(128) NOT NULL,
  `texto` TEXT NOT NULL,
  `posicao` SMALLINT UNSIGNED NOT NULL,
  `id_secao` INT UNSIGNED NOT NULL,
  ...
  CONSTRAINT `fk_secoes_textos_secoes1`
    ...
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = 'Tabela de textos das secoes';
...
CREATE TABLE IF NOT EXISTS `audioweb`.`usuarios_tokens` (
  ...
  INDEX `fk_usuarios_tokens_usuarios1_idx` (`usuarios_id_usuario` ASC),
